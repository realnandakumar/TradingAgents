  ...
  return (
    <div className="space-y-0 -mx-4 sm:-mx-6 max-w-none">
      <div className="px-4 sm:px-6 py-3 border-b border-border bg-surface flex flex-wrap items-center gap-4">
        <div>
          <div className="text-sm font-semibold tracking-tight">NSE Momentum Desk</div>
          <div className="text-[10px] font-mono text-muted uppercase">MOMENTUM_TRADE_POSITIONAL</div>
        </div>
        ...
        <div className="ml-auto text-right text-xs text-muted">
          ...
          <div className="font-mono text-[10px] truncate max-w-[280px]">{momentumBookPath()}</div>
        </div>
      </div>

      <div className="mx-4 sm:mx-6 mt-4">
        <DeskActionsPanel deskId="momentum" />
      </div>

      {!book ? (
        <div className="card m-4 sm:m-6 p-8 text-center">
          <h2 className="font-medium mb-2">No momentum book found</h2>
          <p className="text-muted text-sm mb-4">
            Run <code className="text-accent">tradingagents momentum-daily</code> to open positions.
          </p>
          <p className="text-muted text-xs font-mono">{momentumBookPath()}</p>
        </div>
      ) : (
        <>
          <div className="card overflow-hidden mx-4 sm:mx-6 mt-4 sm:mt-6">
            <MomentumBlotter positions={book.positions} />
          </div>
          ...
          {open.length === 0 && (
            <p className="text-muted text-sm text-center py-8">No open momentum positions.</p>
          )}
        </>
      )}
    </div>
