  ...
  return (
    <div className="space-y-0 -mx-4 sm:-mx-6 max-w-none">
      ...
      <div className="px-4 sm:px-6 py-3 border-b border-border bg-surface flex flex-wrap items-center gap-4">
        <div>
          <div className="text-sm font-semibold tracking-tight">NSE Swing Desk</div>
          <div className="text-[10px] font-mono text-muted uppercase">SWING_TRADE_POSITIONAL</div>
        </div>
        ...
        <div className="ml-auto text-right text-xs text-muted">
          ...
          <div className="font-mono text-[10px] truncate max-w-[280px]">{swingBookPath()}</div>
        </div>
      </div>
      ...
      {!book ? (
        <div className="card m-4 sm:m-6 p-8 text-center">
          ...
          <p className="text-muted text-sm mb-4">
            Run <code className="text-accent">tradingagents swing-daily</code> to open positions, or
            set <code className="text-accent">SWING_BOOK_PATH</code>.
          </p>
          <p className="text-muted text-xs font-mono">{swingBookPath()}</p>
        </div>
      ) : (
        <>
          <div className="card overflow-hidden mx-4 sm:mx-6 mt-4 sm:mt-6">
            <SwingBlotter positions={book.positions} />
          </div>
          ...
          {open.length === 0 && (
            <p className="text-muted text-sm text-center py-8">No open swing positions.</p>
          )}
        </>
      )}
    </div>
