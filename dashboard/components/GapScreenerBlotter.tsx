...
export function GapScreenerBlotter({ picks }: { picks: GapScreenerPick[] }) {
    ...
    <section className="card overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs font-mono">
          ...
          <tbody>
            {rows.map((p) => (
              <tr key={`${p.symbol}-${p.rank}`} className="border-b border-border/40 last:border-0">
                ...
                <td className="py-2 px-3 text-right tabular-nums">{p.fill_pct.toFixed(0)}%</td>
  ...
  return (
    <div className="space-y-4">
      {downs.length > 0 && renderTable(downs, "Gap Down — bullish fill bias")}
      {ups.length > 0 && renderTable(ups, "Gap Up — bearish fill bias")}
    </div>
  );
}
