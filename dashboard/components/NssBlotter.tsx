...
function tickMark(ok?: boolean): string {
  return ok ? "✓" : "—";
}
      ...
      <div className="grid lg:grid-cols-[1fr_240px]">
        <div className="border-r border-border min-w-0">
          <div className="px-4 py-2.5 border-b border-border/60 flex items-center gap-2 text-xs">
            ...
            <span className="ml-auto text-muted">1D · T1 full exit · 40d max</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs font-mono">
              <thead>
                <tr className="text-muted uppercase tracking-wide border-b border-border bg-surface/40">
                  {["SYM", "SCORE", "NAME", "SECT", ...deskOpenMetaHeaders(), "LAST", "MTM%", "STOP", "RISK%", "T1", "UPSIDE", "CUSH%", "BO", "VOL", "RSI", "ADX"].map(
                    (h) => (
                      <th
                        key={h}
                        className={`font-medium py-2 px-2 whitespace-nowrap ${
                          ["SCORE", "LAST", "MTM%", "STOP", "RISK%", "T1", "UPSIDE", "CUSH%", "RSI", "ADX", ...deskOpenMetaHeaders()].includes(h)
                            ? "text-right"
                            : h === "OK" || h === "BO" || h === "VOL"
                              ? "text-center"
                              : "text-left"
              ...
              <tbody>
                {rows.map(({ p, live, stop, mtmPct, cushion, tight }) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-2 text-center text-bull" title="Breakout (informational)">
                      {tickMark(p.breakout_ok)}
                    </td>
                    <td className="py-2 px-2 text-center text-bull" title="Volume ≥ ref (informational)">
                      {tickMark(p.volume_ok)}
                    </td>
