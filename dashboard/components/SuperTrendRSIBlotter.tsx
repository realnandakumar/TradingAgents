...
export function SuperTrendRSIBlotter({ positions }: { positions: StrsiPosition[] }) {
  ...
  const rows = open.map((p) => {
    ...
    const score = (p as StrsiPosition & { signal_score?: number }).signal_score;
    const flipAge = (p as StrsiPosition & { flip_age?: number }).flip_age;
    return { p, live, stop, mtmPct, cushion, tight, score, flipAge };
  });
      ...
      <div className="grid lg:grid-cols-[1fr_240px]">
        <div className="border-r border-border min-w-0">
          <div className="px-4 py-2.5 border-b border-border/60 flex items-center gap-2 text-xs">
            ...
            <span className="ml-auto text-muted">1D · T1 full · SELL closes · 20d</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs font-mono">
              ...
              <tbody>
                {rows.map(({ p, live, stop, mtmPct, cushion, tight, score, flipAge }) => (
                  <tr
                    key={p.ticker}
                  ...
                  >
                    <td className="py-2 px-3 font-medium">
                      <ChartTickerLink ticker={p.ticker} desk="supertrend-rsi" className="hover:text-accent" />
                    </td>
        ...
        <div className="p-4 space-y-4 text-xs">
          ...
          <div>
            <div className="text-[10px] uppercase tracking-wider text-muted mb-2">Open lines</div>
            {rows.length === 0 ? (
              <p className="text-muted">No open positions.</p>
            ) : (
              <ul className="space-y-2 font-mono">
                {rows.map(({ p, mtmPct, score }) => (
                  <li key={p.ticker} className="flex justify-between gap-2">
                    <span>{shortSymbol(p.ticker)}</span>
                    <span className="text-muted">sc {score ?? "—"}</span>
                    <span className={mtmPct >= 0 ? "text-bull" : "text-bear"}>
                      {pct(mtmPct, 1)}
                    </span>
                  </li>
                ))}
