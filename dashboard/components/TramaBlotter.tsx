...
import type { TramaPosition } from "@/lib/trama-server";
...
export function TramaBlotter({ positions }: { positions: TramaPosition[] }) {
      ...
      <div className="grid lg:grid-cols-[1fr_240px]">
        <div className="border-r border-border min-w-0">
          <div className="px-4 py-2.5 border-b border-border/60 flex items-center gap-2 text-xs">
            ...
            <span className="ml-auto text-muted">1D · TRAMA cross · 20-day hold</span>
          </div>
            ...
            <table className="w-full text-xs font-mono">
              <thead>
                <tr className="text-muted uppercase border-b border-border bg-surface-2/30">
                  ...
                  <th className="text-right py-2 px-2">TRAMA</th>
              ...
              <tbody>
                {rows.map(({ p, live, stop, mtmPct, cushion, tight, crossAge, distPct, trama }) => (
                  <tr
                    key={p.ticker}
