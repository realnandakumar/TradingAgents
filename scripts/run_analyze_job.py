#!/usr/bin/env python3
"""Run a dashboard analysis job from ~/.tradingagents/analyze/jobs/."""
...
# Ensure repo root is on sys.path when invoked from dashboard API.
_REPO_ROOT = Path(__file__).resolve().parent.parent
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

from tradingagents.analysis.runner import (  # noqa: E402
    job_path_for_id,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run a TradingAgents analyze job")
    parser.add_argument("--job-id", help="Job id under ~/.tradingagents/analyze/jobs/")
