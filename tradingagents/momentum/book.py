"""Persist momentum screener picks as ``momentum_trade_positional`` positions."""
...
STRATEGY_NAME = "momentum_trade_positional"


class MomentumPositionBook:
    def __init__(self, config: Optional[dict] = None):
        cfg = config or {}
        self.config = cfg
        path = cfg.get("momentum_book_path") or os.getenv(
            "TRADINGAGENTS_MOMENTUM_BOOK_PATH",
            os.path.join(_DEFAULT_HOME, "momentum", "positions.json"),
        )
    ...
    def _load(self) -> dict:
        if self.path.exists():
            try:
                ...
                logger.warning("Could not read momentum book (%s); starting fresh", e)
    ...
    def save_picks(self, picks: List["MomentumPick"], screen_date: Optional[str] = None) -> List[dict]:
    ...
    def _pick_to_position(self, pick: "MomentumPick", screen_date: str) -> dict:
