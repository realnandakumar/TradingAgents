"""Persist NSS screener picks as ``nanda_swing_scanner`` positions."""
...
from .exits import evaluate_momentum_bar_exits
...
STRATEGY_NAME = "nanda_swing_scanner"


class NSSPositionBook:
    def __init__(self, config: Optional[dict] = None):
        cfg = config or {}
        self.config = cfg
        path = cfg.get("nss_book_path") or os.getenv(
            "TRADINGAGENTS_NSS_BOOK_PATH",
            os.path.join(_DEFAULT_HOME, "nss", "positions.json"),
        )
        self.path = Path(path).expanduser()
        self.path.parent.mkdir(parents=True, exist_ok=True)
        self.max_holding_days = int(cfg.get("nss_max_holding_days", 40))
        ...
        self.st_period = int(cfg.get("nss_supertrend_period", 10))
        self.st_mult = float(cfg.get("nss_supertrend_multiplier", 3.0))
    ...
    def _ensure_position_fields(self, p: dict) -> bool:
        ...
        for key, val in defaults.items():
            if key not in p or p[key] is None:
                p[key] = val
                changed = True
        # Legacy runners are force-closed on daily; stop promoting partial state.
    ...
    def _load(self) -> dict:
        if self.path.exists():
            try:
                ...
                logger.warning("Could not read NSS book (%s); starting fresh", e)
    ...
    def open_position(self, pick: "NSSPick", screen_date: str) -> Optional[dict]:
    ...
    def save_picks(self, picks: List["NSSPick"], screen_date: Optional[str] = None) -> List[dict]:
    ...
    def _pick_to_position(self, pick: "NSSPick", screen_date: str) -> dict:
        ...
        return {
            ...
            "primary_reason": pick.primary_reason,
            "rsi": pick.rsi,
            "adx": pick.adx,
            "relative_volume": pick.relative_volume,
            "range_pct": pick.range_pct,
            "atr_ratio": pick.atr_ratio,
            "breakout_ok": getattr(pick, "breakout_ok", False),
            "volume_ok": getattr(pick, "volume_ok", False),
