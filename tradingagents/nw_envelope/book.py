"""Persist NW Envelope screener picks as ``nw_envelope`` positions."""
...
from .trailing import latest_nwe_bands, ratchet_stop
...
STRATEGY_NAME = "nw_envelope"


class NwEnvelopePositionBook:
    def __init__(self, config: Optional[dict] = None):
        cfg = config or {}
        self.config = cfg
        path = cfg.get("nwe_book_path") or os.getenv(
            "TRADINGAGENTS_NWE_BOOK_PATH",
            os.path.join(_DEFAULT_HOME, "nw_envelope", "positions.json"),
        )
        ...
        self.target_1_rr = float(cfg.get("nwe_target_1_rr", 1.5))
        self.target_2_rr = float(cfg.get("nwe_target_2_rr", 2.5))
    ...
    def _pick_to_position(self, pick: "NwEnvelopePick", screen_date: str) -> Optional[dict]:
        sig = pick.signal
        entry = float(sig.close)
        lower = float(sig.lower)
        # BUY below lower band: close < lower, so use symmetric risk below entry.
        stop = lower if lower < entry else round(2 * entry - lower, 2)
        if stop <= 0 or stop >= entry:
            logger.warning(
                "Invalid trade levels for %s (entry=%s lower=%s stop=%s)",
                pick.symbol, entry, lower, stop,
            )
            return None
    ...
    def _update_trailing_stop(self, p: dict, history: pd.DataFrame) -> None:
        mid, low, up = latest_nwe_bands(
            history, bandwidth=self.bandwidth, mult=self.mult, lookback=self.lookback
        )
        if low is None:
            return
        close = float(history["Close"].iloc[-1])
        trail_ref = max(low, mid) if mid is not None else low
        if close > trail_ref:
            p["trailing_stop"] = round(
                ratchet_stop(float(p.get("trailing_stop") or p["stop_loss"]), trail_ref),
                2,
            )
            p["stop_loss"] = p["trailing_stop"]
            p["nwe_lower"] = round(low, 2)
            if mid is not None:
                p["nwe_middle"] = round(mid, 2)
            if up is not None:
                p["nwe_upper"] = round(up, 2)
