"""Portfolio paper-trade manager for NW Envelope."""
...
from .book import NwEnvelopePositionBook
...
class NwEnvelopePaperTradeManager:
    def __init__(self, config: Optional[dict] = None):
        cfg = config or {}
        self.config = cfg
        self.book = NwEnvelopePositionBook(cfg)
        self.max_positions = int(cfg.get("nwe_max_positions", 20))
        self.max_per_sector = int(cfg.get("nwe_max_per_sector", 4))
        nwe_dir = Path(
            cfg.get("nwe_book_path", os.path.join(_DEFAULT_HOME, "nw_envelope", "positions.json"))
        ).parent
        self.pending_path = Path(cfg.get("nwe_pending_path") or nwe_dir / "pending_replacements.json")
        self.daily_dir = Path(cfg.get("nwe_daily_dir") or nwe_dir / "daily")
    ...
    def propose_replacement(self, pick: "NwEnvelopePick", today_tickers: set[str]) -> ReplacementProposal:
    ...
    def approve_replacement(self, proposal_id: str, pick: "NwEnvelopePick") -> bool:
