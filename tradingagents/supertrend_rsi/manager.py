"""Portfolio paper-trade manager for supertrend_rsi."""
...
class SuperTrendRSIPaperTradeManager:
    def __init__(self, config: Optional[dict] = None):
        ...
        self.pending_path = Path(cfg.get("strsi_pending_path") or strsi_dir / "pending_replacements.json")
        self.daily_dir = Path(cfg.get("strsi_daily_dir") or strsi_dir / "daily")
    ...
    def _weakest_open(self, today_tickers: set[str]) -> Optional[dict]:
        ...
        def score(p: dict) -> float:
            ...
            if p["ticker"] not in today_tickers:
                s -= 5.0
            s += float(p.get("signal_score") or 0)
            s -= float(p.get("flip_age") or 0)
            return s
    ...
    def propose_replacement(self, pick: "SuperTrendRSIPick", today_tickers: set[str]) -> ReplacementProposal:
        ...
        return ReplacementProposal(
            ...
            close_score=float(weak.get("signal_score") or 0),
            new_ticker=pick.symbol,
            new_stock_name=pick.signal.stock_name or pick.symbol,
            new_signal_score=pick.signal.score,
            new_flip_age=pick.signal.flip_age,
        )
