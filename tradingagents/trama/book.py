"""Persist TRAMA crossover screener picks as ``trama_crossover`` positions."""
...
from .trailing import latest_trama_line, ratchet_stop
from tradingagents.paper.json_store import aggregate_closed_trades, dump_json, sanitize_for_json
...
# Tests import this name from trama.book.
...
class TramaPositionBook:
    def __init__(self, config: Optional[dict] = None):
        cfg = config or {}
        self.config = cfg
        path = cfg.get("trama_book_path") or os.getenv(
            "TRADINGAGENTS_TRAMA_BOOK_PATH",
            os.path.join(_DEFAULT_HOME, "trama", "positions.json"),
        )
        self.path = Path(path).expanduser()
        self.path.parent.mkdir(parents=True, exist_ok=True)
        self.holding_days = int(cfg.get("trama_holding_days", 20))
        self.max_positions = int(cfg.get("trama_max_positions", 20))
        self.max_per_sector = int(cfg.get("trama_max_per_sector", 4))
        self.desk_capital = float(cfg.get("desk_capital", 100_000.0))
        self.trama_length = int(cfg.get("trama_length", 100))
        self.target_1_rr = float(cfg.get("trama_target_1_rr", 1.5))
        self.target_2_rr = float(cfg.get("trama_target_2_rr", 2.5))
    ...
    def _backfill_sectors(self) -> None:
        """Fill missing sector from universe metadata (legacy positions used '—')."""
        need = [
        ...
        try:
            ...
            logger.warning("Could not backfill TRAMA sectors (%s)", e)
            return
        changed = False
        for p in need:
            ...
            if sector:
                p["sector"] = sector
                if not p.get("stock_name") or p.get("stock_name") == p["ticker"]:
                    name = meta.get("name")
                    if name:
                        p["stock_name"] = name
                changed = True
    ...
    def _save(self) -> None:
        ...
        payload = sanitize_for_json(self._state)
    ...
    def open_position(self, pick: "TramaPick", screen_date: str) -> Optional[dict]:
    ...
    def save_picks(self, picks: List["TramaPick"], screen_date: Optional[str] = None) -> List[dict]:
        screen_date = screen_date or datetime.now().strftime("%Y-%m-%d")
    ...
    def _pick_to_position(self, pick: "TramaPick", screen_date: str) -> Optional[dict]:
        sig = pick.signal
        entry = float(sig.close)
        stop = float(sig.trama)
        levels = compute_trade_levels(entry, stop, self.target_1_rr, self.target_2_rr)
        if levels is None:
            logger.warning("Invalid trade levels for %s (entry=%s trama=%s)", pick.symbol, entry, stop)
            return None
        ...
        return {
            ...
            "cross_age": sig.cross_age,
            "dist_pct": sig.dist_pct,
            "trama_value": sig.trama,
            "remark": sig.remark,
    ...
    def _update_trailing_stop(self, p: dict, history: pd.DataFrame) -> None:
        ...
        if trama_val is None:
            return
        close = float(history["Close"].iloc[-1])
        if close > trama_val:
            p["trailing_stop"] = round(
                ratchet_stop(float(p.get("trailing_stop") or p["stop_loss"]), trama_val),
                2,
            )
            p["stop_loss"] = p["trailing_stop"]
            p["trama_value"] = round(trama_val, 2)
