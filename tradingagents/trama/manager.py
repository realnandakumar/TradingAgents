"""Portfolio paper-trade manager for TRAMA crossover."""
...
from .book import TramaPositionBook
...
class TramaPaperTradeManager:
    def __init__(self, config: Optional[dict] = None):
        cfg = config or {}
        self.config = cfg
        self.book = TramaPositionBook(cfg)
        self.max_positions = int(cfg.get("trama_max_positions", 20))
        self.max_per_sector = int(cfg.get("trama_max_per_sector", 4))
        trama_dir = Path(
            cfg.get("trama_book_path", os.path.join(_DEFAULT_HOME, "trama", "positions.json"))
        ).parent
        self.pending_path = Path(cfg.get("trama_pending_path") or trama_dir / "pending_replacements.json")
        self.daily_dir = Path(cfg.get("trama_daily_dir") or trama_dir / "daily")
    ...
    def _weakest_open(self, today_tickers: set[str]) -> Optional[dict]:
        ...
        def score(p: dict) -> float:
            ...
            if p["ticker"] not in today_tickers:
                s -= 5.0
            s += float(p.get("dist_pct") or 0)
            s -= float(p.get("cross_age") or 0)
            return s
    ...
    def propose_replacement(self, pick: "TramaPick", today_tickers: set[str]) -> ReplacementProposal:
        ...
        return ReplacementProposal(
            ...
            close_score=float(weak.get("dist_pct") or 0),
            new_ticker=pick.symbol,
            new_stock_name=pick.stock_name or pick.symbol,
            new_signal_score=float(pick.signal.dist_pct),
            new_flip_age=pick.signal.cross_age,
        )

    def approve_replacement(self, proposal_id: str, pick: "TramaPick") -> bool:
    ...
    def run_daily(
        ...
        report["exits"] = legacy + exit_summary.get("closed", []) + sell_closes
